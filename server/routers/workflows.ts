...
export const workflowsRouter = router({
  list: workspaceProcedure.input(workspaceInput.extend({ limit: z.number().int().min(1).max(100).default(50) })).query(async ({ ctx, input }) => { const db = await requireDb(); return db.select().from(workflows).where(and(eq(workflows.workspaceId, ctx.workspaceId), isNull(workflows.deletedAt))).orderBy(desc(workflows.updatedAt)).limit(input.limit); }),
