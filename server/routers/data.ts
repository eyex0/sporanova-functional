...
export const dataSourcesRouter = router({
  ...
  disconnect: workspaceManagerProcedure.input(workspaceInput.extend({ dataSourceId: z.number().int().positive() })).mutation(async ({ ctx, input }) => { const db = await requireDb(); const source = (await db.select().from(dataSources).where(and(eq(dataSources.id, input.dataSourceId), eq(dataSources.workspaceId, ctx.workspaceId), isNull(dataSources.deletedAt))).limit(1))[0]; if (!source) throw new TRPCError({ code: "NOT_FOUND", message: "Data source not found in this workspace." }); await db.update(dataSources).set({ status: "disconnected" }).where(and(eq(dataSources.id, input.dataSourceId), eq(dataSources.workspaceId, ctx.workspaceId))); await writeAuditLog({ workspaceId: ctx.workspaceId, actorUserId: ctx.user.id, action: "data_source.disconnected", resourceType: "dataSource", resourceId: input.dataSourceId }); return { success: true }; }),
  delete: workspaceManagerProcedure.input(workspaceInput.extend({ dataSourceId: z.number().int().positive() })).mutation(async ({ ctx, input }) => { const db = await requireDb(); const source = (await db.select().from(dataSources).where(and(eq(dataSources.id, input.dataSourceId), eq(dataSources.workspaceId, ctx.workspaceId), isNull(dataSources.deletedAt))).limit(1))[0]; if (!source) throw new TRPCError({ code: "NOT_FOUND", message: "Data source not found in this workspace." }); await db.update(dataSources).set({ status: "disconnected", configuration: null, deletedAt: new Date() }).where(and(eq(dataSources.id, source.id), eq(dataSources.workspaceId, ctx.workspaceId))); await writeAuditLog({ workspaceId: ctx.workspaceId, actorUserId: ctx.user.id, action: "data_source.deleted", resourceType: "dataSource", resourceId: source.id }); return { success: true }; }),
});
